...
def allGroups_slim_obtain_filters_mask(model, truncated_layer,groups_thre, CBL_idx, prune_idx):
    ...
    for idx in CBL_idx:
        ...
        if idx <  truncated_layer[0]:
            ...
            if idx - 1 == truncated_layer[2] or idx - 3 == truncated_layer[2]:
            ...
            if idx  == truncated_layer[3] or idx - 1 == truncated_layer[3]:
    ...
    prune_ratio = pruned / total
    print(f'Prune channels: {pruned}\tPrune ratio: {prune_ratio:.8f}')
...
def prune_and_eval(model, CBL_idx, CBLidx2mask):
    ...
    print(f'mask the gamma as zero, mAP of the model is {mAP:.6f}')
...
if __name__ == '__main__':
    ...
    # truncated_layer = [56, 86, 117, 137, 162]
    truncated_layer = [35, 61, 84, 96, 106]
    ...
    if  already_get_num == len(bn_weights):
         print(" the  first sparte groups  this  step   tempolary  pass  pass  \n")

    else:
        print(" the  first ,sparte groups  this  step  not  pass, please check the sparate  group step \n")
        assert already_get_num == len(bn_weights)


    print(" \n  group1,   numbers of  channels  in group1  are = ", len(group1_bn_weights))
    print(" \n  group2,   numbers of  channels  in group2  are = ", len(group2_bn_weights))
    print(" \n  group3,   numbers of  channels  in group3  are = ", len(group3_bn_weights))
    print(" \n  group4,   numbers of  channels  in group4  are = ", len(group4_bn_weights))
    print(" \n  group5,   numbers of  channels  in group5  are = ", len(group5_bn_weights))
    ...
    g1_pruned_percent = 0.32
    g2_pruned_percent = 0.55
    g3_pruned_percent = 0.95
    g4_pruned_percent = 0.85
    g5_pruned_percent = 0.50
    ...
    print('\n using or_Mask_merge the mask of layers connected to shortcut!:')
    ...
    compact_model_name = opt.weights.replace(opt.weights.split('/')[-1], f'group_slim_OrMask_prune_for_darknet_v3_{opt.percent}.weights')
...
'''
python group_slim_OrMask_prune_for_darknet_v3.py  --data data/voc.data  --cfg cfg/yolov3/yolov3-voc.cfg  --weights weights/darknet_v3/d_s0p003_map66_dark_v3.pt
